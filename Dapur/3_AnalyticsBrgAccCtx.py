...
def main():
    ...
    if os.path.exists(config_file):
        ...
        print(f"--> Peringatan: File '{config_file}' tidak ditemukan. Berjalan tanpa alias eksternal.")
    ...
    col_map_core = {
        ...
        "tgl": "Tanggal Faktur Pajak",
        "ppn": "PPN",
    }
    for key, col_name in col_map_core.items():
        if col_name not in df_coretax.columns:
            possible = [c for c in df_coretax.columns if key in c.lower()]
            if possible:
                col_map_core[key] = possible[0]
    c_faktur, c_nama, c_tgl, c_ppn = (
        col_map_core["faktur"],
        col_map_core["nama"],
        col_map_core["tgl"],
        col_map_core["ppn"],
    ...
    df_coretax["clean_name"] = df_coretax[c_nama].apply(normalize_name, alias_dict=alias_map)
    ...
    col_map_acc = {
        "tgl": "Tgl. Pajak",
        "faktur": "No. Faktur Pajak",
        "nama": "Nama Pemasok",
        "ppn": "Jumlah Pajak",
    }
    a_faktur, a_nama, a_tgl, a_ppn = (
        col_map_acc["faktur"],
        col_map_acc["nama"],
        col_map_acc["tgl"],
        col_map_acc["ppn"],
    ...
    df_accurate["clean_name"] = df_accurate[a_nama].apply(normalize_name, alias_dict=alias_map)
    ...
    detail_view_final = add_total_row(
        detail_view.drop(columns=["Grouping_Date"]), "Nama Penjual", numeric_cols
    )
    ...
    try:
        ...
        print("--> Selesai! File 'Analisis_Barang.xlsx' berhasil dibuat.")
    except Exception as e:
        print(f"--> Gagal menyimpan Excel: {e}")
        print(f"--> Pastikan file Excel output tidak sedang dibuka.")
